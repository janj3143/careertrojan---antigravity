...
if not (PORTAL_BRIDGE_AVAILABLE and intelligence_service):
    st.error("❌ Career Intelligence backend is not available in this environment.")
    st.caption("Configure the backend bridge and required endpoints, then retry.")
    st.stop()
            ...
            with st.spinner("🧠 Analyzing career path with AI..."):
                ...
                if PORTAL_BRIDGE_AVAILABLE and intelligence_service:
                    try:
                        ...
                        with col1:
                            st.metric("Current Role", trajectory_data.get('current_role') or "–")
                        with col2:
                            st.metric("Years Experience", trajectory_data.get('years_experience') or "–")
                        with col3:
                            st.metric("Career Level", trajectory_data.get('career_level') or "–")
                        ...
                        st.caption("No fallback analysis is available. Fix backend connectivity and retry.")
...
with tab2:
    ...
    if st.button("🎬 Run Simulation", type="primary", use_container_width=True):
        st.error("❌ Scenario simulation is unavailable until the backend provides a simulation endpoint.")
        st.caption("No simulated results are shown.")
...
with tab3:
    ...
    if st.button("🎨 Generate Cloud", type="primary", use_container_width=True):
        st.info("Cloud overlays are generated in Universal Cloud Maker.")
        if st.button("☁️ Open Universal Cloud Maker", key="open_universal_cloud_maker"):
            st.switch_page("pages/universal_cloud_maker.py")
