...
if st.button("🎨 Generate Cloud", type="primary", use_container_width=True):
    with st.spinner("☁️ Generating cloud visualization..."):
        ...
        if data_source == "My Resume":
            if not st.session_state.get('resume_data') and not st.session_state.get('uploaded_resume_content'):
                st.warning("⚠️ No resume data found. Upload your resume first.")
        ...
        if not (SHARED_IO_AVAILABLE and io_layer):
            st.error("❌ Cloud data service is unavailable in this environment.")
            st.stop()

        user_id = st.session_state.get('user_id')
        if not user_id:
            st.error("❌ Missing user identity. Please log in again.")
            st.stop()

        try:
            candidate_data = io_layer.get_candidate_data(user_id)
        except Exception as exc:
            st.error(f"❌ Failed to load your profile data: {exc}")
            st.stop()

        profile = (candidate_data or {}).get('profile')
        if not isinstance(profile, dict):
            st.warning("⚠️ No profile data available yet. Complete resume analysis first.")
            st.stop()

        if cloud_type == "Skills Cloud":
            words = profile.get('skills') or []
        elif cloud_type == "Technologies Cloud":
            words = profile.get('technologies') or profile.get('skills') or []
        elif cloud_type == "Job Titles Cloud":
            words = profile.get('job_titles') or []
        elif cloud_type == "Industry Keywords Cloud":
            words = profile.get('industry_keywords') or profile.get('industries') or []
        else:
            words = []

        if not words:
            st.warning("⚠️ No data available for this cloud type.")
            st.stop()

        word_freq = {str(word): 1.0 for word in words if str(word).strip()}
        if not word_freq:
            st.warning("⚠️ No valid items found to build a cloud.")
            st.stop()
