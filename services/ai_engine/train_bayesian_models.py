...
class BayesianModelTrainer:
    ...
    def __init__(self, base_path: str = None):
        ...
        try:
            ...
            self.models_path = _cfg_models / "bayesian"
        except ImportError:
            # Fallback: resolve from env or default to L: drive
            import os
            ...
            self.models_path = Path(base_path or Path(__file__).parent) / "trained_models" / "bayesian"
    ...
    def load_training_data(self):
        ...
        logger.info(f"✅ Loaded {len(all_profiles)} profiles")
        return all_profiles
    ...
    def train_naive_bayes(self, X, y):
        ...
        try:
            ...
            logger.info(f"✅ Gaussian Naive Bayes - Accuracy: {acc_gnb:.4f}")
            ...
            logger.info(f"✅ Multinomial Naive Bayes - Accuracy: {acc_mnb:.4f}")
            ...
            logger.info(f"✅ Bernoulli Naive Bayes - Accuracy: {acc_bnb:.4f}")
    ...
    def train_bayesian_network(self, X, y):
        ...
        try:
            ...
            logger.info("✅ Bayesian Network built and saved")
            return {'status': 'success', 'structure': bayesian_net}

        except Exception as e:
            logger.error(f"❌ Bayesian Network failed: {e}")
            return None
    ...
    def train_hierarchical_bayesian(self, X, y):
        ...
        try:
            ...
            logger.info(f"✅ Hierarchical Bayesian Model - CV Score: {scores.mean():.4f}")
            return {'cv_score': scores.mean(), 'cv_std': scores.std()}

        except Exception as e:
            logger.error(f"❌ Hierarchical Bayesian failed: {e}")
            return None
    ...
    def train_mcmc_sampler(self, X, y):
        ...
        try:
            ...
            logger.info("✅ MCMC Sampler configuration saved")
            return mcmc_config

        except Exception as e:
            logger.error(f"❌ MCMC setup failed: {e}")
            return None
